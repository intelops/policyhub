...
#ApplicationSourceKustomize: {
	...
	patches?: #KustomizePatches @go(Patches) @protobuf(12,bytes,opt)

	// Components specifies a list of kustomize components to add to the kustomization before building
	components?: [...string] @go(Components,[]string) @protobuf(13,bytes,rep)

	//LabelWithoutSelector specifies whether to apply common labels to resource selectors or not
	labelWithoutSelector?: bool @go(LabelWithoutSelector) @protobuf(14,bytes,opt)
}
...
#KustomizePatch: {
	...
	patch?:  string                    @go(Patch) @protobuf(2,bytes,opt)
	target?: null | #KustomizeSelector @go(Target,*KustomizeSelector) @protobuf(3,bytes,opt)
	options?: {[string]: bool} @go(Options,map[string]bool) @protobuf(4,bytes,opt)
}

#KustomizeSelector: {
	#KustomizeResId
	annotationSelector?: string @go(AnnotationSelector) @protobuf(2,bytes,opt)
	labelSelector?:      string @go(LabelSelector) @protobuf(3,bytes,opt)
}
...
#RevisionHistory: {
	...
	// InitiatedBy contains information about who initiated the operations
	initiatedBy?: #OperationInitiator @go(InitiatedBy) @protobuf(10,bytes,opt)
}
...
#AWSAuthConfig: {
	...
	profile?: string @go(Profile) @protobuf(3,bytes,opt)
}
